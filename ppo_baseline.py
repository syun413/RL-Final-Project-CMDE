...
if wandb_log:
    import wandb
    wandb.init(project="ppo_reach_target_sparse_var_2_new", config={
...
for update in range(num_updates):
    ...
    for step in range(NUM_STEPS):
        ...
        if done:
            ...
            video_filename = f"rollout_videos_sparse_var2/rollout_{rollout_count}{video_suffix}.mp4"

            import moviepy.video.io.ImageSequenceClip
            from moviepy.editor import vfx
            os.makedirs("rollout_videos_sparse_var2", exist_ok=True)
    ...
    if (update + 1) % CHECKPOINT_INTERVAL == 0:
        os.makedirs(
            "./baseline_checkpoints_sparse_var2", exist_ok=True)
        checkpoint_path = f"./baseline_checkpoints_sparse_var2/ppo_checkpoint_{update+1}.pth"
